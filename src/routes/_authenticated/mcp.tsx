...
type Server = { id: string; name: string; url: string; status: "active" | "idle" | "error"; tools: number };
...
function McpView() {
  ...
  const disconnect = (id: string) => {
    ...
    if (!target) return;
    if (!confirm(`Disconnect ${target.name}?`)) return;
    setServers((prev) => prev.filter((s) => s.id !== id));
    toast.success(`${target.name} disconnected`, {
      action: {
        label: "Undo",
        onClick: () => setServers((prev) => (prev.some((s) => s.id === id) ? prev : [...prev, target])),
      },
    });
  };
  return (
          ...
          {servers.map((s) => (
            <div key={s.id} className="px-5 py-3 flex items-center gap-4">
              ...
              <button
                ...
                className="h-7 px-2.5 rounded-md text-[12px] border border-[var(--border-default)] text-[var(--text-secondary)] hover:text-[var(--danger)] hover:border-[var(--danger)]/40"
              >
                Disconnect
              </button>
            </div>
